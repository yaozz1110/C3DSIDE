...
def compute_errors(gt, pred):
    ...
    # RMSE
    rmse = (gt / 1000 - pred / 1000) ** 2
    rmse1 = np.sqrt(rmse.mean())
    ...
    sq_rel = np.mean(((gt / 1000 - pred / 1000) ** 2) / (gt / 1000))

    # Mean Squared Error (MSE)
    mse = np.mean((gt/1000 - pred/1000) ** 2)

    # Mean Absolute Error (MAE)
    mae = np.mean(np.abs(gt/1000 - pred/1000))
    ...
    return abs_rel, sq_rel, rmse1, rmse_log, a1, a2, a3, mse, mae, r_squared
...
def convert_disps_to_depths_kitti(gt_depth, pred_disparities):
    ...
    for i in range(len(pred_disparities)):
        ...
        pred_depth = width_to_focal * 60 / (pred_disp + (1.0 - mask))
        # pred_depth = width_to_focal * 60 / pred_disp
